...
describe('SecretHandshakeManager', () => {
    ...
    it('should has a particular code named reverse', () => {
        let shm = new SecretHandshakeManager()
        expect(shm.REVERSE).to.equal(shm.REVERSE)
    })
    ...
    it('should return the list of codes, without apply particular codes', () => {
        ...
        expect(shm.getListOfCodes(16)).to.eql([shm.REVERSE])

        expect(shm.getListOfCodes(3)).to.eql(['wink', 'double blink'])
        expect(shm.getListOfCodes(19)).to.eql(['wink', 'double blink', shm.REVERSE])
    })
    ...
    it('should concat the lists of codes', () => {
        let shm = new SecretHandshakeManager()
        //19 = [19]
        expect(shm.manageListOfCodes([19])).to.eql(['wink', 'double blink', shm.REVERSE])
        //123 = [27, 3]
        expect(shm.manageListOfCodes([27, 3])).to.eql(['wink', 'double blink', 'jump', shm.REVERSE, 'wink', 'double blink'])
        //1123 = [3, 3, 1]
        expect(shm.manageListOfCodes([3, 3, 1])).to.eql(['wink', 'double blink', 'wink', 'double blink', 'wink'])
        //528 = [16, 16]
        expect(shm.manageListOfCodes([16, 16])).to.eql([shm.REVERSE, shm.REVERSE])
        //627 = [19, 19]
        expect(shm.manageListOfCodes([19, 19])).to.eql(['wink', 'double blink', shm.REVERSE, 'wink', 'double blink', shm.REVERSE])
    })
